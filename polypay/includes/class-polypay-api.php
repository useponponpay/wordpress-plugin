...
class PolyPay_API
{
	...
	public function __construct($api_key, $api_url = '', $timeout = 30)
	{
		...
		$this->api_url = $api_url ?: POLYPAY_API_URL;
	...
	public static function get_debug_log_file()
	{
		...
		$base_dir = trailingslashit($upload_dir['basedir']) . 'polypay';
		...
		return trailingslashit($base_dir) . 'polypay-debug.log';
	}
	...
	private function request($endpoint, $data = [])
	{
		...
		$args = [
			...
			'headers' => [
				...
				'User-Agent' => 'WordPress-PolyPay/' . POLYPAY_VERSION,
			],
		...
		if ($http_code !== 200) {
			...
			$this->write_debug_log('');
			return new WP_Error('polypay_api_error', $error_msg, ['status' => $http_code]);
		}

		if (json_last_error() !== JSON_ERROR_NONE) {
			$this->write_debug_log("[$request_id] JSON Decode Error: " . json_last_error_msg());
			$this->write_debug_log('');
			return new WP_Error('polypay_json_error', 'Invalid JSON response');
		}
